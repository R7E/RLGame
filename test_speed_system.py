...
color_light_wall = libtcod.Color(29, 48, 89)
color_index = 0
color_index1 = 0
color_index2 = 0
...
class Object:
    ...
    def move(self, dx, dy):
        ...
        if not is_blocked(self.x + dx, self.y + dy):
            self.x += dx
            self.y += dy
        self.wait = self.speed #might need to change indent check out how it works both ways
...
def make_map():
    ...
    for r in range(1):
        ...
        w = SCREEN_WIDTH - 4 #libtcod.random_get_int(0, ROOM_MIN_SIZE, ROOM_MAX_SIZE)
        h = SCREEN_HEIGHT - 10  #libtcod.random_get_int(0, ROOM_MIN_SIZE, ROOM_MAX_SIZE)
        #random position without going out of the boundaries of the map
        x = 2 #libtcod.random_get_int(0, 0, MAP_WIDTH - w - 1)
        y = 2 #libtcod.random_get_int(0, 0, MAP_HEIGHT - h - 1)
...
def handle_keys():
    ...
    if game_state == 'playing':
        if player.wait > 0: #don't take turn yet if still waiting
            #player.wait -= 1
            print 'player wait in handle keys %d' % player.wait
            return 'wait'
        #movement keys
...
libtcod.console_init_root(SCREEN_WIDTH, SCREEN_HEIGHT, 'test speed system', False)
libtcod.sys_set_fps(LIMIT_FPS)
con = libtcod.console_new(SCREEN_WIDTH, SCREEN_HEIGHT)
 
#create object representing the player
fighter_component = Fighter(hp=30, defense=2, power=5, death_function=player_death)
player = Object(0, 0, '@', 'player', libtcod.white, blocks=True, fighter=fighter_component, speed=DEFAULT_SPEED)
...
fov_recompute = True
game_state = 'playing'
player_action = None # initiate player action variable
...
while not libtcod.console_is_window_closed():
    ...
    if game_state == 'playing': #and player_action == 'didnt-take-turn': #comment out and to make real time
        for object in objects:
            if object.ai:
                #object.ai.take_turn()
                if object.wait > 0: #don't take turn yet if still waiting
                    object.wait -= 1
                    #print (object.name.capitalize() + ' wait')
                else:
                    object.ai.take_turn()
                    #print (object.name.capitalize() + ' turn')